...
Component({
    ...
    methods: {
      ...
      updateMonth(year, month, day) {
        const curDay = day || undefined;
        this.setMonth(year, month, curDay);
        const selectDay = this.data.selectDay.day || new Date(this.data.defaultTime).getDate();
        const newMonth = month < 10 ? `0${month}`: month;
        this.witchDate(new Date(`${year}-${newMonth}-${selectDay}`));
      },
      //上月切换按钮点击
      lastMonth() {
